...
public class QuestionBatchController {
    ...
    public ResponseEntity<byte[]> downloadTemplate() throws IOException {
        ...
        ResponseEntity<byte[]> responseEntity = ResponseEntity.ok()
                .header("Content-Disposition", "attachment; filename=excel-template.xlsx")
    ...
    public Result<List<QuestionImportVo>> previewExcel(
        ...
        List<QuestionImportVo> questions = questionService.previeExcel(file);
        log.info("生成预览数据结束，数据为：{}" ,questions);
    ...
    public Result<List<QuestionImportVo>> generateQuestionsByAi(
        ...
        List<QuestionImportVo> questions = questionService.aiGenerateQuestions(request);
        log.info("使用ai生成题目数据成功，需要生成的题目标题为{}，需要生成的题目数量为{}，实际生成的题目为{}",
                request.getTopic(), request.getCount(), questions.size());
        return Result.success(questions);
    }
